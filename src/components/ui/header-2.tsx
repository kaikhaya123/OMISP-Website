...
{ label: 'OMISP Capital', href: '/capital' },
...
/>
<span className={cn('font-tanker font-bold text-xl md:text-2xl leading-none', scrolled ? 'text-black' : 'text-white')}>
OMISP
</span>
</Link>
...
<div
className={cn('fixed inset-0 z-[200] md:hidden', open ? 'flex' : 'hidden')}
>
{/* Full Screen Video Background */}
<div className="absolute inset-0 w-full h-full overflow-hidden bg-black">
<video autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover" src="/Videos/0_Business_Meeting_Presentation_1080x1920 (1).mp4" />
...
<Link to="/signup" onClick={() => setOpen(false)} className="mt-4 md:mt-4">
...
<p className="text-xs md:text-xs text-white">{user.email}</p>
</div>
)}
<div className="text-right text-sm text-white/80">Designed for founders</div>
</div>
</div>
</div>
</>
